...
exports.callback = function(err) {
  ...
  console.log("抱歉，%s失败：%s", err.action || '操作', err.message || err);
  debug(err.stack);
  process.exit(1);
};
...
exports.request = function(router, options, cb) {
  ...
  var url = options.apiServer || AV.serverURL;
...
exports.getRedisInstances = function(cb) {
  var url = AV.serverURL + '/1.1/__cache/ops/instances';
  request({
    url: url,
    method: 'GET',
    headers: {
...
exports.requestRedis = function(server, db, command, cb) {
  var url = AV.serverURL + '/1.1/__cache/ops/instances/' + server + '/dbs/' + db;
  request({
    url: url,
    method: 'POST',
    headers: {

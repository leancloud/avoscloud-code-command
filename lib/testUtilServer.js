...
app.get('/__engine/1/functions', function(req, res) {
  getFunctions(function(funcName) {
    ...
    res.send(data.map(function(name) {
      return {
        name: name,
        sign: signHook(app.get('masterKey'), name, Date.now())
      }
    }));
  });
});
  ...
  getFunctions(function(name) {
    ...
    data = _.map(data, function(name) {
      ...
      if (name == '__on_login__User') {
        ...
        result.sign = signHook(app.get('masterKey'), '__on_login__User', Date.now());
      }
  ...
  request({
    ...
    if (response.statusCode === 200) {
      try {
        ...
        console.log('云函数列表解析失败: responseBody=%s, err=%s', body, err);
        cb(null, []);
      }
    } else {
      console.log('云函数调试工具查询云函数列表失败: statusCode=%d', response.statusCode);
      debug('查询云函数列表响应：\n%s', body);
      cb(null, []);
    }
  });

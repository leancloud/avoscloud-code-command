...
function watchFile(f, name){
	...
	fs.watchFile(f,{ persistent: true, interval: 2000 },function(curr, prev){
		if(curr.mtime != prev.mtime){
			...
			if(f.toString().indexOf("app.js")==-1){
			  console.log("File cloud/app.js automatically reload...");
				loadRelativeFile("cloud/app.js");
			}
	...
	var processRequest0 = function(user){
		...
		if(type == 'object'){
			...
			var mockResp =new MockResponse({
				...
				error: function(err){
					console.log("Error occured:" + err);
					res.send({code:1 , error: err.message || err});
				}
			});
...
function loadRelativeFile(name){
	var cloudPath = path.resolve(Global.rootPath + name);
	requireFromFile(cloudPath, name);
}

exports.runCloudCode = function(rootPath, masterKey){
    ...
    //Load main.js
	loadRelativeFile('cloud/main.js');

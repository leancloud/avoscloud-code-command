...
if (AV.User._old_saveCurrentUser == null) {
  ...
  AV._saveCurrentUser = function(user, old) {
    var req;

    if (old) {
      AV._old_saveCurrentUser(user);
    } else {
      if ((AV.Cloud.__express_req != null) && (user != null)) {
        req = AV.Cloud.__express_req;
        if (req._avos_session._uid != null) {
          req._avos_session._uid = user.id;
          req._avos_session._sessionToken = user._sessionToken;
        }
      }
    }
    return AV._old_saveCurrentUser(user);
  };
}

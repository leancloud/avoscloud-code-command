...
var _ref, _ref1;
if ((_ref = https.globalAgent)) {
    if ((_ref1 = _ref.options)) {
        _ref1.rejectUnauthorized = false;
    }
}


function MockRequest(object, params, user, remoteAddress) {
    this.object = object;
...
AV.Cloud.beforeUpdate = function(name, func) {
    Global.funcs[className(name) + "_beforeUpdate"] = func;
};
...
AV.Cloud.onLogin = function(func) {
    Global.funcs[className('_User') + '_onLogin'] = func;
};

AV.Cloud.run = function(name, params, options) {
  var promise = new AV.Promise();
  if (!Global.funcs[name]) {
     return promise.reject("Could not find function:" + name);
  }
  var mockReq = new MockRequest(null, params, AV.User.current());
  var newOptions = {
  ...
  var mockResp = new MockResponse(newOptions);
  runFunc(name, mockReq, mockResp);
...
function runHooker(fname) {
    return function(name, req, res) {
      runFunc(className(name) + fname, req, res);
    };
}
...
AV.Cloud.httpRequest = function(options) {
    ...
    var contentLen = theBody ? theBody.length : 0;
...
function requireFromFile(path, filename) {
    ...
    cache[path] = m;
    m._compile("var AV = require('avoscloud-sdk');var __production=0; var __local = true; " + src, filename);
    return m.exports;
}
...
Module.prototype.require = function(id) {
    ...
    if (id === 'avos-express-cookie-session') {
      return avosExpressCookieSession;
    }
    if (id === 'avos-express-https-redirect') {
      return avosExpressHttpsRedirect;
    }
    var result = Module._load(id, this);
    if (id == 'express') {
        var oldExpress = result;
        result = function() {
            if (Global.app) {
            ...
            app.listen = function() {
                var views = path.join(Global.rootPath, this.get('views'));
                var publicDir = path.join(Global.rootPath, 'public');
                this.set('views', views);
                this.use(oldExpress.static(publicDir));
                this.use(function(err, req, res, next) {
                    if (err) {
                        console.error("Error occured:", err.stack || err);
                        return res.send(err);
                    } else {
                        return next();
                    }
    ...
    var processRequest0 = function(user) {
        ...
        if (type == 'object') {
            ...
            createObject(req, res, function(object) {
                var mockReq = new MockRequest(object, null, user, req.ip);
                ...
                var target = runHooker('_' + func);
                target.call(this, className, mockReq, mockResp);
            });
        } else {
            var mockReq = new MockRequest(null, req.body.params || req.body, user, req.ip);
            var mockResp = new MockResponse({
                success: function(data) {
                    res.send({
                        result: data
...
AV.User.logInByIdAndSessionToken = function(uid, sessionToken, fetch, cb) {
  var user;
  fetch = fetch || false;
  user = new AV.User();
  user.id = uid;
...
AV.User.logOut = function() {
  ...
  if (process.domain) {
    currentUser = process.domain._currentUser;
  }
  if (currentUser) {
...
AV.User._saveCurrentUser = function(user) {
  if (process.domain && process.domain._currentUser == user) {
    return;
  }
  if (process.domain && process.domain._currentUser) {
    AV.User.logOut();
  }
  if (user) {
